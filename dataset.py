...
class DataSet:
    def __init__(self, in_dir, exts='.jpg'):
        ...
        # Filenames for all the files in the pred-set.
        self.filenames_pred = []
        ...
        for name in os.listdir(in_dir):
            ...
            if os.path.isdir(current_dir):
                ...
                # Get all the valid filenames in the sub-dir named 'test'.
                filenames_pred = self._get_filenames(os.path.join(current_dir, 'predict'))
                self.filenames_pred.extend(filenames_pred)

                # Create an array of class-numbers.
                class_numbers = [class_number] * len(filenames_pred)

                # Append them to the list of all class-numbers for the test-set.
                self.class_numbers_pred.extend(class_numbers)
    ...
    def get_paths(self, test=False, pred=False):
        """
        ...
        if pred and test == False :
            filenames = self.filenames_pred
            class_numbers = self.class_numbers_pred

            pred_dir = "predict/"
            test_dir = ""

        elif test and pred== False:
        ...
        for filename, cls in zip(filenames, class_numbers):
            ...
            path = os.path.join(self.in_dir, self.class_names[cls], test_dir, pred_dir, filename)
    ...
    def get_pred_set(self):
        ...
        return list(self.get_paths(test=False, pred=True)), \
               np.asarray(self.class_numbers_pred), \
               one_hot_encoded(class_numbers=self.class_numbers_pred,
                               num_classes=self.num_classes)

    def copy_files(self, train_dir, test_dir, pred_dir):
        """
        ...
        _copy_files(src_paths=self.get_paths(test=False, pred=False),
                    dst_dir=train_dir,
                    class_numbers=self.class_numbers)
        ...
        _copy_files(src_paths=self.get_paths(test=True, pred=False),
                    dst_dir=test_dir,
                    class_numbers=self.class_numbers_test)
        ...
        # Copy the files for the test-set.
        _copy_files(src_paths=self.get_paths(test=False, pred=True),
                    dst_dir=pred_dir,
                    class_numbers=self.class_numbers_pred)

        print("- Copied pred-set to:", pred_dir)
